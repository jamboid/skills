...
The style branch of [`draft-document`](SKILL.md), read before drafting and applied again in the tells pass. Every rule governs the document being written — a human reader is the target, not another agent.
...
- **Headings name what the section settles.** "Deploying to staging", "Who owns what". Replaces: Overview, Introduction, Key considerations, Final thoughts, Conclusion.
...
- **Write to the reader, never about them.** Every sentence addresses the audience named in the brief. Anything about how that audience will receive the document belongs in the conversation with whoever commissioned it. Replaces the **audience aside** — "this is the win that will sell the approach", "the client will appreciate the transparency here". The tell is a sentence that only makes sense if the reader is standing outside the document looking at it.

...
On mobile this audit scores 0 (it was 0.5 on desktop). The per-asset data pins it exactly: the three sample banners all ship as `width_scale_xl` on desktop (~276–314 KB each), and on mobile two of them **correctly drop to `width_scale_m` (297→87 KB and 276→43 KB) — but the LCP hero stays at 314 KB.** The reason is F2: the hero is a CSS background (`b_media__bg`), and backgrounds get no responsive `srcset`/derivative, so the phone is forced to download the full desktop image.

The images are **already WebP**, so this is a sizing problem, not a format one. Give the hero a responsive treatment (the same image styles its siblings already use, ideally via a real `<img srcset>` per F2); that alone reclaims ~227 KB and ~0.3 s of LCP on mobile. AVIF would shave a little more, but sizing is the lever.

**F3** — Server is on HTTP/1.1 — no multiplexing _(est. mobile LCP −0.85 s · FCP −0.6 s)_
...
**F4** — Text assets served without compression _(free win)_
...
**F6** — Images lack explicit width and height _(CLS insurance)_
...
2. **Turn on HTTP/2 and text compression**  _(Impact: High · Effort: Low)_

   The server still speaks HTTP/1.1, so requests queue head-of-line instead of loading in parallel — Lighthouse attributes about 850 ms of mobile LCP to that alone. Enabling HTTP/2 and gzip/Brotli compression on text assets is a server-configuration change with no code impact.

   Related findings: F3, F4

3. **Unblock first paint in the page head**  _(Impact: Medium · Effort: Low)_

   Stylesheets and scripts in the document head hold back the first paint. Inlining the critical CSS and deferring the JavaScript shortens the blank-screen window at the start of every visit by roughly 600 ms on mobile.

   Related findings: F5

4. **Confirm the remaining imagery is right-sized**  _(Impact: Low · Effort: Low)_

   The other banners already serve correctly-sized images, but it's worth verifying each breakpoint across the rest of the page. The images are already WebP; moving to AVIF would shave a little more if further savings are needed.

   _Strategic priority_

5. **Lock in layout stability, then validate with field data**  _(Impact: Low · Effort: Low)_

   Adding explicit width/height to images keeps the page's already-good layout stability from regressing as content changes. With that in place, collecting real-world field/CrUX data confirms the lab mobile-LCP picture against actual visitors before sign-off.
